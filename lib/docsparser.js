...
DocsParser.prototype.format_function = function(name, args, retval, options) {
    ...
    if(options.hasOwnProperty('as_setter')) {
        out.push('${1:Private}: ${1:' + description + '}');
    } else {
      out.push('${1:Public}: ${1:' + description + '}');
    }
    ...
    if(args) {
        ...
        for (i = 0; len = parsed_args.length,i < len; i++) {
            ...
            format_str = this.editor_settings.param_format;
            if(this.editor_settings.param_description)
                format_str += '${1:[description]}';
            format_str = format_str.replace('${1:[type]}', type_info);
            format_str = format_str.replace('${1:[arg_name]}', escape(arg_name));
            out.push(format_str);
        }
    }
